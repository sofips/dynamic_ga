...
def reward_based_fitness_optimized(
    ...
    for action in action_sequence:
        ...
        fid = np.abs(state[n - 1])**2  # Equivalent to `real(state * conjugate(state))`
...
def reward_based_fitness_vectorized(action_sequence, props, tolerance, reward_decay, test_normalization=True
):
    # compute states
    n = np.shape(props)[1]
    state = np.zeros(n, dtype=np.complex_)
...
def calculate_reward(states,tolerance,reward_decay):
    ...
    rewards[(fid > 0.8) & (fid <= 1 - tolerance)] = 100 / (1 + np.exp(10 * (1 - tolerance - fid[(fid > 0.8) & (fid <= 1 - tolerance)])))
...
for n in dimensions:
    ...
    for iteration in range(iterations):
        ...
        start = time.time()
        fitness = reward_based_fitness_optimized(action_sequence, props, tolerance=0.95, reward_decay=0.95)
        end = time.time()
    ...
    for iteration in range(iterations):
        ...
        start = time.time()
        fitness = reward_based_fitness_vectorized(action_sequence, props, tolerance=0.95, reward_decay=0.95)
        end = time.time()
    ...
    for iteration in range(iterations):
        ...
        start = time.time()
        fitness = reward_based_fitness_original(action_sequence, props, tolerance=0.95, reward_decay=0.95)
        end = time.time()
